---
name: generic-staged-validation-commit-message
description: Validates and fixes syntax/semantics and linter issues in the current solution delivery across languages/stacks, drafts a commit message, then optionally asks to commit and merge into a validation branch (review/staging/qa/test). Use for non–Enonic-XP-backend work or mixed deliveries.
---
...
This skill is not read-only. Find issues **and apply fixes** before reporting.

For Enonic XP 7.16 **backend** JavaScript CommonJS rules, prefer `enonic-staged-validation-commit-message` for the backend subset; this skill still runs generic syntax/semantic checks on all delivery files.

Never run `git commit` or merge during validation (steps 1–9). Optional **Steps 10–11** ask the user before any git write.
...
2. **Inspect delivery diff** — read changed hunks **and full file context** around edits.
3. **Group files** by kind (source, tests, config, CI, docs) and note stack from repo cues.
4. **Syntax & semantic validation** — run checks below per file type; record every issue.
5. **Apply fixes** — fix blockers and safe issues in delivery scope (see Fix policy).
6. **Re-validate** — repeat until PASS or only unfixable blockers remain.
7. **Linter diagnostics** — `ReadLints` on touched source files after fixes.
8. **Optional tooling** — run project checks when quick and available (`tsc --noEmit`, `node --check`, etc.).
...
| Check | What to do |
|-------|------------|
| **Unclosed brackets** | Balance `{}`, `[]`, `()`; fix mismatched or dangling braces. |
| **Missing semicolons / statement termination** | Fix where required by project style or where ASI would break (e.g. line starts with `(`, `[`, `` ` `` after a statement). |
| **Parse/syntax errors** | Run language-appropriate checks when available (`node --check` for JS, JSON.parse for JSON, YAML load for YAML); fix reported errors. |
| **Undefined / missing functions & symbols** | Confirm calls, imports, and identifiers resolve (in-file, imported module, or known global); fix typos, missing imports, or wrong names. |
| **Unresolved imports/requires** | Verify paths and exported names exist; fix broken references. |
| **Dangling references** | After renames/removals, grep for leftover symbols, routes, config keys. |
| **Dead or orphaned fragments** | Remove leftover lines from partial refactors (variables/functions referenced but never defined). |

Read the **whole file** when the diff touches control flow, functions, or braces — not only changed lines.

## Validation by file type

Adapt depth to file type. Always fix blockers before reporting PASS.
...
| **Typed languages** | Run `tsc`/compiler check when project has it; fix errors in delivery scope. |
| **All text** | Secrets/credentials in diff, wrong paths, obvious dead code in changed hunks. |
| **Docs / markdown** | Broken links in changed sections; no fabricated changelog entries. |

If the delivery is empty, say so and skip validation (tell the user which files to stage manually if work exists unstaged).

## Fix policy

Apply fixes when they:

- Fix syntax/semantic **Blockers** (unclosed braces, parse errors, undefined symbols, broken imports).
- Resolve linter **errors** in delivery files when the fix is clear and local.
- Remove orphaned/dead fragments introduced in the delivery.
...
Prefer the **smallest fix** that makes the file valid.

After each fix pass, re-read the file and re-run validation checks.

## Linter checks

- Run `ReadLints` on touched source files **after** fixes.
- Fix linter **errors** in the delivery when the fix is clear and local.
- Summarize remaining warnings; fix only when trivial and in changed code.
- If diagnostics may be stale, re-read the file and note the risk.
...
- Bullets summarize meaningful staged deltas (include validation fixes if part of this delivery).
